...
function PoetryPage() {
  ...
  useEffect(() => {
    apiClient.browseContent({ contentType: "poem" })
  ...
  return (
    <PlatformShell
      title="Poetry"
      subtitle="Verses that breathe, lines that linger — multilingual poetic expressions."
    >
      {loading ? (
        <div className="flex items-center gap-2 py-16 justify-center text-foreground/50">
          <Loader2 className="w-4 h-4 animate-spin" />
          <span className="font-serif-lit italic">Loading poetry…</span>
        </div>
      ) : items.length === 0 ? (
        <div className="paper-texture rounded-xl border border-foreground/10 p-6 shadow-paper">
          <p className="font-serif-lit italic text-foreground/70">No poems published yet.</p>
        </div>
      ) : (
